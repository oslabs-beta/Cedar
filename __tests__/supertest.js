...
describe('Route integration', () => {
  ...
  describe('/api/aws/getFunctionNames', () => {
    describe('GET', () => {
      it('responds with 200 status and content type', () => { 
        return request(server)
        .get('/api/aws/getFunctionNames')
...
describe('Route integration', () => {
  ...
  describe('/api/aws/getMetricData', () => {
    describe('POST', () => {
      const metricsData = {
        start: 1651592607701,
        end: 1652802207701, 
        funcs: ["test-func-app-HelloWorldFunction-2DJ2VlqjVGLQ", "logsdata", "iterateLoop"],
        metrics: ["Invocations", "Throttle", 'Errors']
      };
      //our response should be an object
      it('responds with 200 and JSON application type', () => {
        return request(server)
        .post('/api/aws/getMetricData')
        .send(metricsData)
        .expect('Content-Type', /application\/json/)
        .expect(200);
      });
      ...
      it('responds with a 500 if there is a server error', () => {
